...
const static NSString *kModuleName = @"CAIAudioCapturer";

static AudioUnit m_audioUnit;
static AudioBufferList *m_buffList;
static AudioStreamBasicDescription m_audioDataFormat;

@interface CAIAudioCapturer ()

@property (nonatomic, assign, readwrite) BOOL isRunning;

@end

@implementation CAIAudioCapturer

static CAIAudioCapturer *sharedInstance = nil;
...
// 未调用 configureWithSampleRate: 配置时返回 nil（对其发消息为空操作），
// 避免以非法采样率创建 AudioUnit。
+ (instancetype)sharedCapturer {
    if (!_isConfigured) {
        NSLog(@"%@: audio capturer is not configured, call configureWithSampleRate first \n", kModuleName);
        return nil;
    }
...
- (void)configureAudioToAudioFormat:(AudioStreamBasicDescription *)audioFormat
    ...
    // 采样率与声道数由调用方指定（来自 AVAudioSession 的当前配置），不再查询硬件参数
