...
@property (nonatomic, strong) UIButton *distributeAppBtn;
@property (nonatomic, strong) UIButton *pauseBtn;
@property (nonatomic, strong) UIButton *stopControlBtn;
...
@implementation CAIDemoSettingView
...
- (void)initSubViews {
    ...
    self.restartBtn = [self createBtnFrame:CGRectMake(selfWidth - 80, 80, 70, 25) title:@"重启云端应用"];
    self.distributeAppBtn = [self createBtnFrame:CGRectMake(left + 90, 180, 70, 25) title:@"分发App"];
    self.pauseBtn = [self createBtnFrame:CGRectMake(selfWidth - 80, 120, 70, 25) title:@"挂起控制"];
    self.stopControlBtn = [self createBtnFrame:CGRectMake(selfWidth - 80, 160, 70, 25) title:@"结束控制"];
    ...
    [self addSubview:self.distributeAppBtn];
    [self addSubview:self.pauseBtn];
    [self addSubview:self.stopControlBtn];
...
- (void)controlBtnClick:(id)sender {
    if (sender == self.enableKeyboardBtn) {
    ...
    } else if (sender == self.stopControlBtn) {
        self.hidden = YES;
        [self.delegate stopControl];
    } else if (sender == self.restartBtn) {
        [self.delegate restartCloudApp];
    } else if (sender == self.distributeAppBtn) {
        [self.delegate distributeApp];
        ...
        [self.delegate pauseResumeControl:isPause];
...
- (void)setAllDebugInfo:(NSDictionary *)allInfo {
    self.allInfo.text = [[allInfo description] stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
}
