...
#import "TCGCameraVideoCapturer.h"
...
#include "gcd_helpers.h"

const int64_t kNanosecondsPerSecond = 1000000000;

@interface TCGCameraVideoCapturer
...
@property(nonatomic, weak) TcrSession* tcrSession;
@end

@implementation TCGCameraVideoCapturer {
...
- (instancetype)initWithTcrSession:(TcrSession*)tcrSession {
  return [self initWithTcrSession:tcrSession captureSession:[[AVCaptureSession alloc] init]];
}
...
- (void)dealloc {
  NSAssert(!_willBeRunning,
           @"Session was still running in TCGCameraVideoCapturer dealloc. Forgot to "
           @"call stopCapture?");
  [[NSNotificationCenter defaultCenter] removeObserver:self];
}

+ (NSArray<AVCaptureDevice *> *)captureDevices {
#if defined(__IPHONE_10_0) && defined(__IPHONE_OS_VERSION_MAX_ALLOWED) && \
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
  ...
  int64_t timestampNs = CMTimeGetSeconds(CMSampleBufferGetPresentationTimeStamp(sampleBuffer)) *
    ...
    NSLog(@"sendCustomVideoPixelBuffer");
    [_tcrSession sendCustomVideoPixelBuffer:pixelBuffer rotation:_rotation captureTimeNs:timestampNs];
}
...
#endif
  NSLog(@"Dropped sample buffer. Reason: %@", (__bridge NSString *)droppedReason);
}
...
- (void)setupVideoDataOutput {
  ...
  NSSet<NSNumber *> *supportedPixelFormats =
      [TCGCameraVideoCapturer supportedPixelFormats];
...
- (void)updateVideoDataOutputPixelFormat:(AVCaptureDeviceFormat *)format {
  ...
  if (![[TCGCameraVideoCapturer supportedPixelFormats] containsObject:@(mediaSubType)]) {
