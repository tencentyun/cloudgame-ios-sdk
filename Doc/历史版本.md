[English document](Release_Notes_EN-US.md)

### Version 3.17.0
**Features**
- `TcrSdkInstance` 新增 `setAccessToken:token:error:`，用于设置云手机实例的 AccessInfo 与 Token。凭证按实例 ID 增量合并，续期时不影响已创建的会话，可在凭证即将过期时重复调用。（2026-09-15）
- `TcrSession` 新增云手机应用管控接口：`distributeApp:`（下发并安装指定包名的 APK，结果经 `DISTRIBUTE_STATUS_CHANGED` 事件通知）、`preserveApps:`（只保留指定应用）、`keepAppInForeground:`（保持指定应用常驻前台）、`disableForegroundApp`（关闭常驻前台）。（2026-09-15）
- `TcrEvent` 新增 `DISTRIBUTE_STATUS_CHANGED`（应用下发状态变化）事件。（2026-09-15）
- `TcrSession` 的 STATE_CLOSED 事件新增连接失败细分错误码：`SessionStopConnectFailedDisconnected`(104011)、`SessionStopConnectFailedNetwork`(104012)、`SessionStopConnectFailedResponseInvalid`(104013)、`SessionStopServerSessionParseFailed`(104014)、`SessionStopConnectFailedInvalidState`(104015)、`SessionStopConnectFailedAccessInfoMissing`(104016)、`SessionStopLocalSdpSetFailed`(104018)、`SessionStopRemoteSdpSetFailed`(104019)、`SessionStopLocalSdpCreateFailed`(104020)、`SessionStopClientSessionInvalid`(104021)。（2026-09-15）
- 云手机 Demo（TCAIDemo）重写：登录页、实例列表页、实例操作页、串流页按页面拆分，并补充实例属性、应用管理、文件与截图等接口的调用示例。（2026-09-16）

**Bug Fixes**
- 修复重复设置凭证会导致已创建会话的凭证失效、进而连接失败的问题。（2026-09-12）

**特别注意（行为变更）**
- **SDK 支持的最低系统版本由 iOS 12.0 提升至 iOS 15.0。** 若您的应用仍需支持 iOS 12~14，请继续使用 3.16.0。升级时需同步把工程的 iOS Deployment Target 调整为 15.0 及以上，否则 `pod install` 会直接报错。
- `TcrSession` 的 `start:` 返回值由 `BOOL` 改为 `void`。连接结果一律通过 `TcrSessionObserver` 的 `onEvent` 回调返回：成功为 STATE_CONNECTED，失败为 STATE_CLOSED 并携带具体错误码。原返回值只表示「请求是否已发出」，无法代表连接结果，容易被误当成连接成功。若您的代码使用了该返回值，请改为在 `onEvent` 中判断。
- 凭证接口收敛为 `setAccessToken:token:error:`，以下接口已移除：`TcrConfig` 类、`setTcrConfig:error:`、`updateToken:`、`updateInstanceAccessInfo:token:error:`。原先两步（构造 `TcrConfig` 再设置）改为一步调用，首次设置与后续续期都用同一个接口。
- `TcrEnvTest` 不再是公开类。该类仅用于 Demo 请求体验服务器，不属于 SDK 接入能力，请勿在业务代码中使用。
- 连接失败时，SDK 不再回调 `SessionStopConnectFailedSdp`(104010)，改为回调上述 SDP 相关细分错误码。该常量作为兼容保留，但不会再出现。

### Version 3.16.0
**Features**
- `TcrEvent` 新增 `TOKEN_EXPIRED`（鉴权 token 过期）事件。（2026-07-03）
- 云手机实例鉴权对齐：SDK 按实例 InstanceId 保存鉴权信息，并在单机/群控连接时透传实例鉴权信息。（2026-07-31）
- 音频会话参数化：`TcrSession initWithParams:andDelegate:` 新增 `audioSessionCategoryOptions`、`audioSessionMode` 参数，自定义 SDK 操作 AVAudioSession 的 category options 与 mode，便于与第三方音频 SDK 共存。（2026-08-12）
- 更新 TWEBRTC.framework。（2026-08-12）
- `TcrSession` 的 STATE_CLOSED 事件新增连接失败细分错误码：`SessionStopConnectFailedServerRejected`(104009)、`SessionStopConnectFailedSdp`(104010)、`SessionStopServerSessionInvalid`(104008)。（2026-08-13）

**Bug Fixes**
- 修复自定义音频采集的问题。（2026-08-12）
- 修复 Demo 层登录失败和 SDK 层单机/群控连接失败的问题。（2026-07-31）

**特别注意（行为变更）**
- 连接失败时，SDK 不再回调 SessionStopConnectFailed(104007)，改为回调上述细分错误码。若您的代码对 104007 有精确匹配判断，请适配为细分错误码，或将 104007 作为未知连接失败的兜底处理。
- 云端返回的 ServerSession 无效时，SDK 不再回调 SessionStopServerUnknown(104001)，改为回调 SessionStopServerSessionInvalid(104008)。

### Version 3.15.12 (2026-4.21)
Bug Fixes Gamepad

### Version 3.15.9 (2026-4.2)
Bug Fixes
- 重连问题。

### Version 3.15.8 (2026-3.9)
Bug Fixes
- 摄像头释放问题。

### Version 3.15.7 (2026-2.3)
Bug Fixes
- H264 上行 Profile 太低的问题。

### Version 3.15.2 (2026-1.21)
Features
- TcrSession新增参数sessionMode

### Version 3.15.1 (2025-12.11)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.15.0 (2025-12.11)
Features
- TcrSession新增inputText:mode:indexAfterOverride接口

### Version 3.14.0 (2025-12.10)
Features
- TcrSession新增setRemoteVideoProfile:minBitrate:maxBitrate:width:height接口

### Version 3.13.1 (2025-12.9)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.13.0 (2025-12.8)
Features
- TcrSdkInstance新增updateInstanceAccessInfo:token:error接口

### Version 3.12.2 (2025-12.5)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.12.1 (2025-11.19)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.12.0 (2025-11.19)
Features
- TcrSession新增上行视频弱网降级策略接口，可选择【不降级】、【维持帧率】、【维持分辨率】、【平衡】策略
- CLIENT_STATS新增上行分辨率、上行丢包率、上行编码码率指标

Bug Fixes
- 修复和优化一些已知问题

### Version 3.11.0 (2025-10.29)
Features
- CLIENT_STATS事件新增音视频上行帧率数据

### Version 3.10.3 (2025-10.27)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.10.2 (2025-9.4)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.10.1 (2025-9.1)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.10.0 (2025-8.30)
Features
- 通过 PROXY_RELAY_AVAILABLE 事件传递代理中继信息

### Version 3.9.1 (2025-8.22)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.9.0 (2025-8.20)
Features
- 新增TcrSession sendCustomVideoPixelBuffer:rotation:captureTimeNs接口，用于发送自定义采集视频帧。此能力需要通过TcrSession initWithParams的enableCustomVideoCapture字段开启生效。

### Version 3.8.3 (2025-8.14)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.8.2 (2025-8.12)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.8.1 (2025-8.11)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.8.0 (2025-8.6)
Features
- TcrRenderView新增resetRenderState接口，用于重置onFirstFrameRendered事件的触发

### Version 3.7.1 (2025-8.5)
Bug Fixes
- 修复和优化一些已知问题

### Version 3.7.0 (2025-8.1)
Features
- 优化麦克风权限请求时机，按需请求，而不是在连接建立后立即请求。

### Version 3.6.0 (2025-7.24)
Features
- 新增云手机接口
- 新增TcrEvent的MIC_STATUS_CHANGED事件

### Version 3.5.0 (2025-7.8)
Features
- 新增空闲检测功能，支持在创建 TcrSession 时通过 idleThreshold 参数设置空闲时间阈值。当用户空闲时长达到阈值时，将触发 CLIENT_IDLE 回调。
- 新增onLocationChanged接口设置云端设备经纬度
- 新增onSimulateSensorEvent接口发送模拟传感器事件

### Version 3.4.0 (2024-11.28)
Features  
- 新增传感器数据传递接口MotionSensor

### Version 3.3.1 (2024-11.15)
Features  
- TcrEvent修改CAMERA_STATUS事件关联数据

### Version 3.3.0 (2024-11.15)
Features  
- TcrEvent新增CAMERA_STATUS事件

Bug Fixes 
- 修复和优化一些已知问题

### Version 3.2.5 (2024-6.24)
Bug Fixes 
- 修复某些情况下会触发的crash问题

### Version 3.2.4 (2024-6.19)
Features
- sdk添加隐私清单

### Version 3.2.3 (2024-5.20)
Bug Fixes 
- 修复和优化一些已知问题

### Version 3.2.2 (2024-4.7)
Bug Fixes   
- 修复数据通道发送消息错误的问题

### Version 3.2.1 (2024-3.14)
Features  
- MinimumOSVersion升级为12

### Version 3.2.0 (2024-3.13)
Features  
- TcrEvent新增OPEN_URL事件

### Version 3.1.6 (2024-5.20)
Bug Fixes 
- 修复和优化一些已知问题

### Version 3.1.5 (2024-4.7)
Features  
- 修复数据通道发送消息错误的问题

### Version 3.1.4 (2024-3.25)
Bug Fixes 
- 修复某些情况下会触发的crash问题

### Version 3.1.1 (2024-3.1)
Bug Fixes 
- 修复上报错误  

### Version 3.1.0 (2024-2.22)
Features  
- 新增TcrEnvTest用于demo请求体验使用

### Version 3.0.9 (2024-1.11)
Bug Fixes 
- 修复packet_lost无符号整数显示问题

### Version 3.0.8 (2024-1.10)
Bug Fixes 
- 修复某些情况下MobileTouch失效的问题

### Version 3.0.7 (2023-12.26)
Bug Fixes 
- 修复某些情况下会触发的crash问题

### Version 3.0.6 (2023-12.22)
Bug Fixes 
- 修复和优化一些已知问题

### Version 3.0.5 (2023-12.21)
Refactor  
- 修改AvAudioSession的初始参数

Bug Fixes 
- 修复和优化一些已知问题

### Version 3.0.4 (2023-12.11)
Features  
- 性能数据回调新增字段解码/丢帧fps

### Version 3.0.3 (2023-12.11)
Bug Fixes 
- 修复某些情况下会触发的crash问题

### Version 3.0.2 (2023-12.7)
Bug Fixes 
- 修复虚拟按键库中加载图片资源失败的问题
- 修复和优化一些已知问题

### Version 3.0.1 (2023-12.5)
Bug Fixes 
- 优化重连

### Version 3.0.0 (2023-12.3)
Features  
- 更新底层TWEBRTC库
- 优化重连
- 新增getRequestID接口

Bug Fixes
- 修复和优化一些已知问题

### Version 2.2.0 (2023-11.9)
Features  
- 新增虚拟按键库的适配

Bug Fixes
- 修复和优化一些已知问题

### Version 2.1.8 (2023-11.3)
Bug Fixes
- 修复和优化一些已知问题

### Version 2.1.7 (2023-11.3)
Bug Fixes
- 修复和优化一些已知问题

### Version 2.1.6 (2023-11.3)
Bug Fixes
- 修复性能数据获取错误的问题

### Version 2.1.5 (2023-11.2)
Bug Fixes
- 修复鼠标绝对移动点击问题

### Version 2.1.4 (2023-11.1)
Bug Fixes
- 优化和修复一些已知问题

### Version 2.1.3 (2023-10.31)
Bug Fixes
- 修复偶现数据通道创建失败的问题

### Version 2.1.2 (2023-10.30)
Bug Fixes
- 优化和修复一些已知问题

### Version 2.1.1 (2023-10.20)
Bug Fixes
- 优化和修复一些已知问题

### Version 2.1.0 (2023-10.19)
Features 
- 新增视频上行功能接口
- 新增自采集音频接口 

Bug Fixes
- 修复退后台回到前台时黑屏问题
- 修复一些已知的问题

### Version 2.0.0 (2023-10.12)
Features 
- SDK重构接口设计

### Version 1.6.17 (2024-12.5)
Bug Fixes
- 修复sdk退出时出现的leaks

### Version 1.6.16 (2024-11.27)
Bug Fixes
- 修复sdk日志上报错误

### Version 1.6.15 (2024-8.13)
Bug Fixes
- 修复未初始化videoView frame大小时出现的渲染问题

### Version 1.6.14 (2024-7.25)
Bug Fixes   
- 修复偶现的死锁crash问题

### Version 1.6.13 (2024-6.19)
Features
- sdk添加隐私清单
  
### Version 1.6.12 (2024-4.19)
Bug Fixes   
- 修复麦克风反复开启关闭后失效的问题
  
### Version 1.6.11 (2024-3.1)
Bug Fixes 
- 修复上报错误  

### Version 1.6.10 (2023-12.5)
Bug Fixes 
- 优化重连

### Version 1.6.9 (2023-11.17)
Bug Fixes
- 修复设置音频播放开关失效的问题

### Version 1.6.8 (2023-11.14)
Bug Fixes
- 修复重连后先行设置的麦克风禁用失效的问题

### Version 1.6.7 (2023-10.31)
Bug Fixes
- 修复偶现数据通道创建失败的问题

### Version 1.6.6 (2023-10.27)
Bug Fixes
- 优化和修复一些已知问题

### Version 1.6.5 (2023-10.27)
Bug Fixes
- 优化了心跳定时器导致主线程死锁的问题

### Version 1.6.4 (2023-10.26)
Bug Fixes
- 优化和修复一些已知问题

### Version 1.6.3 (2023-10.26)
Bug Fixes
- 优化和修复一些已知问题

### Version 1.6.2 (2023-10.20)
Bug Fixes
- 优化和修复一些已知问题

### Version 1.6.1 (2023-10.17)
Bug Fixes
- 修复退后台回到前台时黑屏问题
- 修复开启自定义音频采集时启动参数未被正确传递的问题

### Version 1.6.0 (2023-10.13)
Features
- 新增支持自定义音频采集。

### Version 1.5.1 (2023-9.20)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.5.0 (2023-9.19)
Features  
- TCGGamePlayer新增启动参数 `preferredCodec`，用于设置首选编解码器。如果设置了该字段，会话将尝试使用首选编解码器进行通信，如果首选编解码器不可用，则会使用其他可用的编解码器。如果未设置该字段，则会话将使用默认的编解码器。

### Version 1.4.2 (2023-9.18)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.4.1 (2023-9.15)
Bug Fixes
- 修复某些场景下SDK中定时器失效的问题

### Version 1.4.0 (2023-9.14)
Features  
- TCGGamePlayer新增onGameProcessStopped回调  

Bug Fixes
- 修复网络未恢复时重连返回超时的问题

### Version 1.3.14 (2023-9.13)
Bug Fixes
- 修复重连后音轨音量设置失效的问题

### Version 1.3.13 (2023-9.13)
Features 
- TCGVideoFrame新增时间戳字段

### Version 1.3.12 (2023-9.13)
Bug Fixes
- 修复rtt统计错误的问题

### Version 1.3.11 (2023-9.12)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.3.10 (2023-9.12)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.3.9 (2023-9.7)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.3.8 (2023-9.5)
Features
- TCGGamePlayer#currentStatisReport新增音频下行码率字段
### Version 1.3.7 (2023-9.1)
Bug Fixes
- 修复音量调整接口setVolume失效的问题

### Version 1.3.6 (2023-8.30)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.3.5 (2023-8.25)
Features
- 音视频数据回调接口增加配置开关内部渲染播放
### Version 1.3.4 (2023-8.22)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.3.3 (2023-8.17)
Features
- 新增启动参数`software_aec`用于开启软件回声消除
### Version 1.3.2 (2023-8.14)
Features
- 新增TCGMultiPlayer接口类
- 日志代理模块重构

Bug Fixes
- 修复和优化一些已知问题
### Version 1.3.1 (2023-8.9)
Bug Fixes
- 修复开启麦克风后的回声问题

### Version 1.3.0 (2023-8.9)
Features
- TCGGamePlayer新增接口setVideoSink && setAudioSink用于回调音视频数据
- 新增TCGVideoFrame && TCGAudioFrame

### Version 1.2.2 (2023-8.9)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.2.1 (2023-7.17)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.2.0 (2023-7.6)
Features
- 新增TCGMultiPlayer接口类

### Version 1.1.9.6 (2023-8-3)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.1.9.5 (2023-8-3)
Bug Fixes
- 修复和优化一些已知问题
### Version 1.1.9.4 (2023-8-2)
Features
- 新增TCGAudioSessionDelegate代理，用于将sdk中操作audiosession的参数回调，并停止sdk操作这些参数
- 新增接口setEnableLocalAudio用于开启关闭本地麦克风启闭
### Version 1.1.9.2 (2023-7-13)
Features
- 新增创建TCGGameplayer可配置audioSession参数
Bug Fixes
- 修复和优化一些已知问题
### Version 1.1.9.1 (2023-4-12)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.1.9 (2022-11-15)
Features
- 新增接口TCGGamePlayer.setRemoteDesktopResolution修改云端桌面分辨率

### Version 1.1.8.22 (2022-5-10)
Bug Fixes
- 修复和优化一些已知问题

### Version 1.1.8.21 (2021-12-10)
Features
- webrtc重连接口替换成重连代理接口
- 修复一些潜在的问题

### Version 1.1.7.20 (2021-10-29)
Features
- 完善虚拟按键的一些细节
- 修复性能日志上报异常不停止的问题

