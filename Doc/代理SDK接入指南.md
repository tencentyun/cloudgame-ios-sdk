...
1. **添加依赖**
   ...
   ```ruby
   pod 'TCRPROXYSDK', :git => 'https://github.com/tencentyun/cloudgame-ios-sdk.git', :tag => 'TCRPROXYSDK/1.3.0'
   ```
...
- (void)onEvent:(TcrEvent)event eventData:(id)eventData {
    switch (event) {
        case PROXY_RELAY_AVAILABLE: {
            ...
            BOOL ok = [[Proxy sharedInstance] initWithUploadBandwidth:@"1MB"
                                                    downloadBandwidth:@"4MB"
                                                      relayInfoString:relayInfo];
...
> **建议调用顺序：**  
> 1. 初始化（收到 PROXY_RELAY_AVAILABLE 并调用 `initWithUploadBandwidth:downloadBandwidth:relayInfoString:`）  
> 2. 启动代理（`startProxy`）  
> 3. 停止代理（`stopProxy`）  
...
- (void)onEvent:(TcrEvent)event eventData:(id)eventData {
    if (event == PROXY_RELAY_AVAILABLE) {
        ...
        BOOL ok = [[Proxy sharedInstance] initWithUploadBandwidth:@"1MB"
                                                downloadBandwidth:@"4MB"
                                                  relayInfoString:relayInfoString];
        if (ok) {
            // 2. 设置连接状态监听
            MyProxyConnectionObserver *observer = [[MyProxyConnectionObserver alloc] init];
            [[Proxy sharedInstance] setConnectionDelegate:observer];
...
| ~~`- (BOOL)initWithBandwidth:(nullable NSString *)bandwidth relayInfoString:(NSString *)relayInfoString`~~ | ~~初始化代理服务并限制带宽~~ | ~~bandwidth: @"1MB"/@"500KB"（最大 4MB）<br>relayInfoString: 同上~~ | BOOL   | **已废弃**，请使用下方三参数版本 |
| `- (BOOL)initWithUploadBandwidth:(nullable NSString *)uploadBandwidth downloadBandwidth:(nullable NSString *)downloadBandwidth relayInfoString:(NSString *)relayInfoString` | 初始化代理服务，分别指定上下行带宽 | uploadBandwidth: 上行带宽（本地→云机），如 @"1MB"、@"512KB"，nil 使用默认值 4MB<br>downloadBandwidth: 下行带宽（云机→本地），如 @"4MB"、@"1MB"，nil 使用默认值 4MB<br>relayInfoString: 云端下发的代理中继信息 | BOOL   | 上下行独立限速，互不影响；必须在 startProxy 前调用 |
